...
export type OrganizadorMunicipal = {
  id: number;
  accountId: string;
  name: string;
  events: number;
...
type MunicipalSnapshot = {
  fields: CampoMunicipal[];
  reservations: ReservaMunicipal[];
  organizers: OrganizadorMunicipal[];
};
...
const initialState: MunicipalSnapshot = {
  ...
  reservations: [
    ...
    },
    {
      id: 'legacy-3',
      championshipId: 9,
      championship: 'Torneio Regional 2026',
...
function overlaps(a: ReservaMunicipal, b: ReservaMunicipal) {
  return a.start < b.end && b.start < a.end;
}

export function registrarReservaMunicipal(
  reservation: Omit<ReservaMunicipal, 'status' | 'reason'>,
) {
  load();
  if (snapshot.reservations.some((item) => item.id === reservation.id)) {
    return false;
  }
  const nextReservation: ReservaMunicipal = {
    ...reservation,
    status: 'PENDING',
  };
  persist({
...
export function cancelarReservaMunicipal(id: string) {
  load();
  persist({
    ...snapshot,
...
export function useEstadoOperacionalPrefeitura() {
  const state = useSyncExternalStore(subscribe, getSnapshot, getServerSnapshot);

  return {
    state,
    createField(input: Omit<CampoMunicipal, 'id' | 'status'>) {
      const field: CampoMunicipal = {
        ...input,
    ...
    decideReservation(
      ...
      if (decision === 'APPROVED') {
        if (!respeitaAntecedenciaMinima(`${target.date}T${target.start}:00`)) {
          return 'MINIMUM_NOTICE' as const;
        }
...
export function formatarDataMunicipal(date: string) {
