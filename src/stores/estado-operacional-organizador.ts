...
function idsAtivos() {
  try {
    const raw = window.sessionStorage.getItem(CHAVE_CAMPEONATOS_ATIVOS_MOCK);
...
function carregarStorage() {
  ...
  if (!sessionListenerRegistrado) {
    ...
    window.addEventListener(EVENTO_SESSAO_MOCK_ALTERADA, () => {
  ...
  try {
    ...
    const accountId = window.sessionStorage.getItem(CHAVE_CONTA_ATIVA_MOCK);
  ...
  if (typeof window !== 'undefined') {
    try {
      window.sessionStorage.setItem(
        STORAGE_KEY,
        JSON.stringify({
          accountId: window.sessionStorage.getItem(CHAVE_CONTA_ATIVA_MOCK),
          state: snapshot,
        }),
      );
    } catch {
...
export function useEstadoOperacionalOrganizador(campeonatoId: number) {
