...
import { atletasPublicosMock } from '@/features/publico/mocks/public-fixtures';
import {
  getPublicChampionshipName,
  getMatchVenueName,
  getPublicTeamName,
  publicCatalogMock,
...
export function PartidaDetailPage() {
  ...
  const partida = publicCatalogMock.obterPartida(id);
  if (!partida)
    return (
  ...
  return (
    <div className="mx-auto w-full max-w-[1100px] px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
      <PageHero
        eyebrow={`${getPublicChampionshipName(partida.campeonatoId)} · ${partida.fase}${partida.grupo ? ` · ${partida.grupo}` : ''} · ${partida.rodada}`}
        title="Detalhes da partida"
        description={`${partida.data ? new Date(`${partida.data}T12:00:00`).toLocaleDateString('pt-BR') : 'Data a definir'} · ${getMatchVenueName(partida.campoId)}`}
      />
      <section className="overflow-hidden rounded-[30px] border border-border/70 bg-card shadow-sm">
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-4">
          {[
            ...
            [Clock, 'Horário', partida.hora ?? 'A definir'],
            [MapPin, 'Local', getMatchVenueName(partida.campoId)],
