    ...
    >
      ...
      <div className="min-w-0 border-border/70 sm:border-l sm:pl-5">
        <p className="truncate text-xs font-semibold tracking-[0.1em] text-muted-foreground uppercase">
          {getPublicChampionshipName(partida.campeonatoId)} · {partida.rodada}
        </p>
        ...
        <p className="mt-2 flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span className="truncate">{getMatchVenueName(partida.campoId)}</span>
        </p>
      </div>
...
export function HomePage() {
  const campeonatos = publicCatalogMock
    .listarCampeonatos({
      estado: 'EM_ANDAMENTO',
      ordenacao: 'RECENTES',
    })
