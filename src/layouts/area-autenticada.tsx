...
export function LayoutAreaAutenticada({
  ...
  const drawerBackground = isNavy ? 'bg-navy-dark' : 'bg-green-dark';
  ...
  return (
    <div className="campo-canvas min-h-screen w-full bg-surface text-foreground">
      <a
      ...
      <header className="sticky top-0 z-40 border-b-2 border-green-dark bg-background">
        <div className="mx-auto flex min-h-16 w-full max-w-[1380px] items-center gap-3 px-4 py-2 sm:px-6 lg:px-8">
          <button
            ...
            className={cn(
              'inline-flex min-h-11 min-w-11 items-center justify-center gap-2 rounded-md border border-green-dark bg-transparent px-3 text-sm font-semibold transition-colors duration-150 hover:bg-green-dark hover:text-white',
              accentText,
              interactiveFocus,
            )}
          ...
          >
            <p
              className={cn(
                'font-display text-2xl leading-none font-extrabold tracking-[0.015em] uppercase',
                accentText,
              )}
            >
              CampoLivre
          ...
          <div className="ml-auto flex min-w-0 items-center gap-3 border-l-2 border-accent px-3 py-1">
            <Iniciais
              name={userName}
      ...
      <DialogPrimitive.Root open={menuOpen} onOpenChange={setMenuOpen}>
        <DialogPrimitive.Portal>
          ...
          <DialogPrimitive.Content
            id="profile-sidebar"
            className={cn(
              'campo-lines fixed inset-y-0 left-0 z-[71] flex h-full w-[min(88vw,360px)] flex-col overflow-hidden text-white shadow-2xl outline-none',
              drawerBackground,
            )}
            ...
            <div className="relative z-10 flex h-full flex-col p-4 sm:p-5">
              <div className="mb-8 flex items-start justify-between gap-4 px-1 pt-1">
                ...
                <button
                  ...
                  onClick={() => setMenuOpen(false)}
                  className={cn(
                    'flex min-h-11 min-w-11 items-center justify-center rounded-md border border-white/20 bg-white/8 text-white transition-colors duration-150 hover:bg-white/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-offset-2',
                    drawerRingOffset,
                  )}
              ...
              <div className="mt-auto border-t border-white/20 pt-4">
                ...
                {onSignOut ? (
                  <button
                    type="button"
                    onClick={onSignOut}
                    className="mt-2 flex min-h-11 w-full items-center justify-center gap-2 rounded-md border border-white/20 px-3 text-sm font-semibold text-white transition-colors hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
                  >
      ...
      <main id="conteudo-principal" tabIndex={-1} className="outline-none">
        <div className="mx-auto w-full max-w-[1380px] space-y-8 px-4 py-6 sm:px-6 sm:py-8 lg:px-8 lg:py-10">
          {children}
        </div>
      </main>
    </div>
...
export function CabecalhoPerfil({
  ...
  return (
    <header
      className={cn(
        'campo-lines mb-2 overflow-hidden border-y-2 text-white',
      ...
      <div className="relative px-5 py-7 sm:px-8 sm:py-9 lg:px-10">
        <div className="relative flex items-end justify-between gap-5">
          <div className="min-w-0">
            ...
            <h1 className="text-balance font-display text-4xl leading-none font-bold tracking-[0.01em] uppercase sm:text-5xl">
              {name}
            </h1>
